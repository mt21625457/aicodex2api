...
复用现有配置项 `gateway.openai_ws.client_read_idle_timeout_seconds` 作为 **passthrough 读空闲超时（idle timeout）**；当未配置或 <=0 时回退为官方语义默认 `3600s`（1 小时）：

1. 不新增额外配置项
2. 语义为 **idle timeout**：每次收到有效帧时重置读 deadline，超过阈值无帧到达则触发优雅关闭
3. 默认值与 OpenAI WebSocket 连接生命周期语义对齐（1 小时）
...
不需要特殊的"排水"机制：WS 连接自然结束时间受 idle timeout 与上游生命周期语义约束（默认最长 3600s）。
