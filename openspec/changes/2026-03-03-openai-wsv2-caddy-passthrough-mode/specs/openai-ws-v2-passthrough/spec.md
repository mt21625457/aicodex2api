...
- **AND** MUST NOT 调用 `hooks.BeforeTurn` / `hooks.AfterTurn`
...
- **WHEN** 上游 WS 连接断开
- **THEN** 系统 MUST 终止客户端连接（发送 WS close frame，status 1001 Going Away）
...
- **AND** MUST NOT 读写 state store（`ResponseAccount`/`SessionConn`/`SessionLastResponseID`）
