...
func TestOpenAIStreamingFirstTokenMs_StrictTokenEvent(t *testing.T) {
	gin.SetMode(gin.TestMode)
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			StreamDataIntervalTimeout: 0,
...
func TestOpenAIStreamingPassthroughFirstTokenMs_StrictTokenEvent(t *testing.T) {
	gin.SetMode(gin.TestMode)
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			MaxLineSize: defaultMaxLineSize,
	...
	go func() {
		defer func() { _ = pw.Close() }()
		_, _ = pw.Write([]byte("data: {\"type\":\"response.created\"}\n\n"))
		_, _ = pw.Write([]byte("data: {\"type\":\"response.completed\",\"response\":{\"usage\":{\"input_tokens\":3,\"output_tokens\":4,\"input_tokens_details\":{\"cached_tokens\":1}}}}\n\n"))
		_, _ = pw.Write([]byte("data: [DONE]\n\n"))
	}()

	result, err := svc.handleStreamingResponsePassthrough(c.Request.Context(), resp, c, &Account{ID: 9}, time.Now())
	_ = pr.Close()
	require.NoError(t, err)
	require.NotNil(t, result)
	require.Nil(t, result.firstTokenMs)
...
func TestOpenAIBuildUpstreamRequest_OAuthCompactAcceptFollowsStreamFlag(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, "application/json", streamReq.Header.Get("accept"))
}
...
func TestHandleOAuthSSEToJSON_IncompleteEventReturnsProtocolError(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/", nil)
	...
	body := []byte(strings.Join([]string{
		`data: {"type":"response.in_progress","response":{"id":"resp_5"}}`,
		`data: {"type":"response.incomplete","response":{"id":"resp_5","incomplete_details":{"reason":"max_output_tokens"}}}`,
		`data: [DONE]`,
	}, "\n"))
